...
class menuImpl {
  ...
  findMenusByRoldId (roles, type) {
    ...
    const sql = ` select m.* FROM menus m, roles_menus r WHERE m.menu_id = r.menu_id AND r.role_id IN (?) AND type != ${new_type} order by m.menu_sort asc `
        ...
        if (!err) {
          resolve(result.map(e => {
            return {
              ...
              name: e.name, component: e.component, menu_sort: e.menu_sort, icon: e.icon, path: e.path,
